...
def orders_fix():
    """get login info"""
    df = pd.read_csv("../input_data/sample_-_superstore.csv", usecols=['orderid']);
    orders = df.values.tolist()
    orders=sum(orders,[])
    for i in range(len(orders)):
        if orders[i] in orders[:i]:
            orders[i]=orders[i]+i
        with open("../input_data/ordersF.txt", "a") as myfile:
            myfile.write(str(orders[i])+ "\n") 
...
def state_fix():
    ...
    for i in range(len(a)):
        with open("../input_data/statef.txt", "a") as myfile:
            myfile.write(a[i]+ "\n")

def orders_output():
    """output login info"""
    df = pd.read_csv("../input_data/sample_-_superstore.csv");
